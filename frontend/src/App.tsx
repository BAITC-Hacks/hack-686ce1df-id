...
import { Activity, ArrowUpDown, Boxes, CircleHelp, FileSearch, Info, LoaderCircle, Network, RefreshCw, Unplug } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="application">
    ...
    <header className="app-header">
      ...
      <div className="header-actions"><button className="icon-button" title="Обновить набор данных" aria-label="Обновить набор данных" onClick={() => void workspace.refresh()} disabled={loading}><RefreshCw size={17} className={loading ? 'spin' : ''} /></button><ExportMenu runId={dataset?.runId ?? null} onRunConflict={workspace.onRunConflict} /></div>
    </header>
    ...
    <main className="workspace">
      <aside className="navigation-panel" aria-label="Поиск и списки">
        <SearchPanel onSearch={selectNode} disabled={!dataset} />
        ...
        <div className="navigation-content">
          {loading ? <div className="panel-status" role="status"><LoaderCircle className="spin" size={23} /><p>Загружаем списки…</p></div> : error ? <div className="panel-status"><Unplug size={27} /><h2>Данные недоступны</h2><p role="alert">{error}</p><button className="button button-secondary" onClick={() => void workspace.refresh()}><RefreshCw size={15} />Повторить</button></div> : dataset ? <>
            ...
            {tab === 'priorities' ? <PriorityTable nodes={dataset.priorities} selectedGid={selectedGid} onSelect={selectNode} /> : <ClusterList clusters={dataset.clusters} selected={selection.scope} onSelect={scope => void workspace.select(scope)} />}
          </> : null}
        </div>
